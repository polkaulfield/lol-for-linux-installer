...
if app_settings['Gamescope'] == '1':
    ...
    gamescope_process = [gamescope_path, "--"]
    subprocess.run(gamescope_process + wine_process + ["--launch-product=league_of_legends", "--launch-patchline=live"], env=start_game_vars, check=True)
else:
    start_game_vars = dict(os.environ, **env_vars)
